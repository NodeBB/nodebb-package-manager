...
Packages.getPlugins = function(version, callback) {
	...
	async.waterfall([
		...
		function (packages, next) {
			...
			checkUsage(nbbVersion, packages, next);
		},
		function (packages, next) {
...
function checkUsage(nbbVersion, packages, callback) {
	...
	packages.forEach(function(package) {
		...
		multi.zcount('nodebb:' + nbbVersion + ':plugin:' + package.name, yesterday, '+inf');

		// check if the latest version of this plugin is seen working on nbbVersion
		multi2.zscore('plugin:' + package.name + ':versions', nbbVersion + '_' + package.latest);
	});
	async.parallel({
