...
Packages.getPlugins = function(version, callback) {
	...
	if (version) {
		rdb.hgetall('packages:' + version, function(err, pkgTable) {
			...
			getPackagesData(version, Object.keys(pkgTable), function (err, packages) {
				if (err) {
					return callback(err);
				}
		...
		rdb.zrange('packages', 0, -1, function(err, packages) {
			...
			getPackagesData(version, packages, function (err, packages) {
				if (err) {
					return callback(err);
				}
...
function getPackagesData(nbbVersion, packages, callback) {
	var multi = rdb.multi();
	...
	packages.forEach(function(pkgName) {
		...
		multi2.zcount('nodebb:' + nbbVersion + ':plugin:' + pkgName, yesterday, '+inf');
	});
	async.parallel({
		data: function (next) {
			multi.exec(next);
		...
		results.data.forEach(function (data, index) {
			if (data) {
				data.numInstalls = results.counts[index] || 0;
			}
		});
...
Packages.getUsage = function (callback) {
	...
	async.parallel({
		...
		activePlugins: function (next) {
			// TODO: figure out what data to show
			// rdb.zrevrangebyscore('nodebb:version:plugins', '+inf', cutoff, function (err, pluginData) {
			// 	if (err) { return next(err); }
			// 	async.map(pluginData, function (keyName, next) {
			// 		rdb.zcard(keyName, next);
			// 	}, function (err, counts) {
			// 		if (err) { return next(err); }
			// 		const data = {};
			// 		pluginData.forEach(function (keyName, index) {
			// 			const parts = keyName.split(':');
			// 			const nbbVersion = parts[1];
