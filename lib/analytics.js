...
const trendingCache = require('lru-cache')({
	max: 10,
	maxAge: 1000 * 60 * 60 * 6,
});

Analytics.init = function() {
	new cronJob('*/' + (nconf.get('ANALYTICS_PERSIST_FREQ') || '10') + ' * * * * *', Analytics.flush, null, true);
};
...
Analytics.summary.geo = function(callback) {
	...
	rdb.zrangebyscore('ip:recent', from, now, function(err, set) {
		...
		for(var country in hash) {
			if (hash.hasOwnProperty(country)) {
				returnArr.push({
					country: country,
					count: hash[country]
				});
