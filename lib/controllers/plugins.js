...
plugins.list = function (req, res, next) {
...
plugins.rebuild = async (req, res, next) => {
...
plugins.search = function (req, res, next) {
	if (!req.query.id || !req.body.version) {
		return res.status(500).send('invalid-data');
	}

	Packages.search(req.query.id, req.query.version, (err, matches) => {
		if (err) {
			return next(err);
		}
		res.json(matches);
